...
class FtpClient(object):
    ...
    def auth(self):
        ...
        while count < 3:
            ...
            if response.get('status_code') == 200:  # pass auth
                ...
                cache_file_name = ".luffy_db_%s" % self.username
                if not os.path.isfile(cache_file_name):
                    self.shelve_obj = shelve.open(".luffy_db_%s" % self.username)
                else:
                    self.shelve_obj = shelve.open(cache_file_name)
                return True
            else:
    ...
    def unfinished_file_check(self):
        ...
        if list(self.shelve_obj.keys()):
            ...
            while True:
                choice = input("[select file index to re-download, input command: 'back' to continue]").strip()
                if not choice:
                    continue
                if choice == 'back':
                    break
                    ...
                    if choice >= 0 and choice <= index:
                        ...
                        if response.get('status_code') == 401:  # "File exist ,ready to re-send !",
                            ...
                            while recv_size < total_size:
                                ...
                                # 重命名 xx.mp4.download
                                rename = local_filename.rsplit(".", maxsplit=1)
                                os.rename(local_filename, rename[0])
                                # 将缓存文件删除
                                cache_file_name = ".luffy_db_%s" % self.username
        ...
        if self.auth():
            ...
            while True:
                ...
                if user_input.upper() != "Q":
                    ...
                    self.sock.close()
                    exit("thanks to use this rubbish ftp system!")
    ...
    def _put(self, cmd_args):
        ...
        if self.parameter_check(cmd_args, exact_args=1):
            ...
            if os.path.isfile(local_file):
                ...
                self.send_msg('put', file_size=total_size, filename=local_file, md5_value=md5_value)
